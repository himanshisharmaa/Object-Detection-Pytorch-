...
#initialize the dictionary containing model name and it's  corresponding
...
model=MODELS[args['model']](pretrained=True,progress=True,num_classes=len(CLASSES),
                            pretrained_backbones=True).to(DEVICE)
model.eval()
#initialize the video stram, allow the camera sensor to warmup,
...
while True:
    # grab the frame from the threshold video stream and resize it
    # to have a maximum width of 400 pixels.
    frame=vs.read()
    frame=imutils.resize(frame,width=400)
    orig=frame.copy()

    #convert the frames from BGR to RGB channel ordering and change
    ...
    # network to get th detections and predictions
    ...
    for i in range(0,len(detections["boxes"])):
        ...
        if confidence> args['confidence']:
            ...
            cv2.putText(orig,label,(startX,y),cv2.FONT_HERSHEY_COMPLEX,
                        0.5,COLORS[idx],2)
